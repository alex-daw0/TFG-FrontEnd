<template>
  <v-app>
    ...
    <v-main class="contenedor">
      <v-container>
        <h1 class="text-center">Registro de Combustibles</h1>
        ...
        <v-data-table :headers="headers" :items="Combustibles" item-key="id" :items-per-page="pagination.rowsPerPage"
          ...
          <template v-slot:header>
            <thead>
              <tr class="centered-header">
                ...
                <th><v-btn class="AddCombustible" style="background-color: var(--primary-color); color: white; margin:auto" @click="
                  dialogInsert = true;
                AddCombustible();
        ...
        <!-- Modal para inserción de combustibles -->

        <v-dialog v-model="dialogInsert" width="700">
          <v-card>
            <h2 class="text-center">Añadir Combustible</h2>
            <v-card-text>
              <h2 style="color: black; display: none; padding: .7rem; background-color: var(--error);  " class="errores text-center">Debes rellenar
                todos los
                campos</h2>
              <form>
                <v-text-field v-model="insertNombre" label="Matricula"></v-text-field>

              </form>
              <div class="botonesModal">
                <button @click="ConfirmAddCombustible">Confirmar</button>
        ...
        <!-- Fin del modal de inserción de combustibles -->

        <!-- Modal para la edición de combustibles -->

        <v-dialog v-model="dialogUpdate" width="700">
          <v-card>
            <h2 class="text-center">Editar Combustible</h2>
            <v-card-text>
              <h2 style="color: black; display: none; padding: .7rem; background-color: var(--error) " class="errores text-center">Debes rellenar
                todos los
                campos</h2>
        ...
        <!-- Fin del modal para la edición de combustibles -->

        <!-- Modal para eliminación de combustibles -->
        <v-dialog v-model="dialogDelete" width="500">
          <v-card>
            <h2 class="text-center">Quieres borrar este combustible?</h2>
            <v-card-text>
              <form>
        ...
        <!-- Fin de modal para eliminación de combustibles -->
      </v-container>
    </v-main>
    <pie />
  </v-app>
...
export default {
  ...
  data: () => {
    return {
      ...
      perPageChoices: [
        { text: '5 Combustibles/página', value: 5 },
        { text: '10 Combustibles/página', value: 10 },
        { text: '20 Combustibles/página', value: 20 },
      ],
      opcionesOrder: [
  ...
  methods: {
    fetchData() {
      ...
      const url = `https://localhost:7030/Combustible/getCombustibles?idEmpresa=${token.idEmpresa}`;
    ...
    ConfirmEditar() {
      ...
      if (this.nombreEdit == '') {
        ...
        const url = `https://localhost:7030/Combustible/updateCombustible?id=${this.CombustibleAEditar}&idEditor=${token.userId}`;

        const params = {
          id: this.CombustibleAEditar,
    ...
    ConfirmDelete() {
      ...
      axios
        .delete(
          `https://localhost:7030/Combustible/deleteCombustible?id=${this.CombustibleAEliminar}&idBorrador=${token.userId}`
        )
        .then((response) => {
          this.$router.go(0);
        })
    ...
    ConfirmAddCombustible() {
      ...
      if (this.insertNombre == '') {
        ...
        document.querySelector('.errores').style.display = "none";
        const url = `https://localhost:7030/Combustible/postCombustible?idCreador=${token.userId}`;
        axios
          .post(url, params)
          .then((response) => {
            this.$router.go(0);
    ...
    Editar(item) {
      ...
      this.CombustibleAEditar = item.id;
    },
